...
internal sealed class InProcessOAuthMcpServer : IAsyncDisposable
{
    ...
    /// </summary>
    public string? LastRequestedScope => _lastRequestedScopeHolder[0];
    ...
    public static async Task<InProcessOAuthMcpServer> StartAsync(bool advertiseOfflineAccess)
    {
        ...
        app.Use(async (context, next) =>
        {
            if (context.Request.Path.StartsWithSegments("/mcp"))
            {
                ...
                if (presented != $"Bearer {AccessToken}")
                {
                    context.Response.StatusCode = StatusCodes.Status401Unauthorized;
                    context.Response.Headers.WWWAuthenticate =
                        $"Bearer resource_metadata=\"{baseUrlHolder[0]}/.well-known/oauth-protected-resource\"";
                    return;
                }
            }
        ...
        {
            resource = $"{baseUrlHolder[0]}/mcp",
            authorization_servers = new[] { baseUrlHolder[0] },
        ...
        {
            issuer = baseUrlHolder[0],
            authorization_endpoint = $"{baseUrlHolder[0]}/connect/authorize",
            token_endpoint = $"{baseUrlHolder[0]}/connect/token",
            registration_endpoint = $"{baseUrlHolder[0]}/connect/register",
        ...
        app.MapGet("/connect/authorize", (HttpContext context) =>
        {
            ...
            lastRequestedScopeHolder[0] = scope;
        ...
        baseUrlHolder[0] = addresses.Addresses.First().TrimEnd('/');

        return new InProcessOAuthMcpServer(app, baseUrlHolder[0], lastRequestedScopeHolder);
    }
