...
// AC-1397: YouTrack split into a backend part and a UI part, following GitStatus's pilot (AC-1390). Acceptance 1 reads
// the csproj files directly, as the sibling Integration/Provider split tests do; acceptance 2 reads the compiled
// backend assembly's IL, so an Active* call in backend code turns this red with the calling class named.
public class YouTrackPluginUiSplitArchitectureTests
{
    // Acceptance 1: the backend names no Avalonia package, and the UI part never references the backend project.
    [Theory]
    [InlineData("Cockpit.Plugin.YouTrack.csproj", "PackageReference", "Avalonia")]
    [InlineData("UI/Cockpit.Plugin.YouTrack.UI.csproj", "ProjectReference", "Cockpit.Plugin.YouTrack.csproj")]
    public void TheProject_NamesNothingItMustNot(string project, string element, string forbidden)
    {
        var path = Path.Combine(_RepositoryRoot(), "plugins-dev", "Cockpit.Plugin.YouTrack", project);
    ...
    // Acceptance 2 (D6): no method in the backend assembly calls a member that reads or acts on "the active session" —
    // the backend has none; a UI part names the pane it means. The failure lists every calling method by class.
    [Theory]
