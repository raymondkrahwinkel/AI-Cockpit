...
/// <summary>
/// End-to-end loader proof for the Gemini/OpenAI provider plugin (#45, the fase A worked example): loads
/// the real compiled plugin through the actual <see cref="PluginActivator"/>/<see cref="PluginLoadContext"/>
/// and asserts type-identity holds, its metadata is right, and it registers both its session providers
/// (Gemini and OpenAI) via <see cref="ICockpitHost.AddSessionProvider"/> — the seam that would otherwise
/// only be exercised by a hand-written fake. Mirrors <see cref="YouTrackPluginLoadTests"/>.
/// </summary>
public class GeminiProviderPluginLoadTests
{
    [Fact]
    ...
    private static string? _LocatePluginOutput()
    {
        ...
        {
            var candidateRoot = Path.Combine(directory.FullName, "plugins-dev", "Cockpit.Plugin.GeminiProvider", "bin");
            if (Directory.Exists(candidateRoot))
            {
                var dll = Directory
                    .EnumerateFiles(candidateRoot, "Cockpit.Plugin.GeminiProvider.dll", SearchOption.AllDirectories)
                    .FirstOrDefault();
