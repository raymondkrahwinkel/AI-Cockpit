...
public class GitHubIssuesPluginLoadTests
{
    ...
    public void ActivatesAndContributes_WhenBuilt()
    {
        ...
        folder.Should().NotBeNull("the example plugin is built as a test dependency");
        ...
        plugin!.Metadata.Id.Should().Be("github-issues");
        plugin.Metadata.DisplayName.Should().Be("GitHub Issues");
    ...
    private static string? _LocatePluginOutput()
    {
        ...
        {
            var candidateRoot = Path.Combine(directory.FullName, "plugins-dev", "Cockpit.Plugin.GitHubIssues", "bin");
            if (Directory.Exists(candidateRoot))
            {
                var dll = Directory
                    .EnumerateFiles(candidateRoot, "Cockpit.Plugin.GitHubIssues.dll", SearchOption.AllDirectories)
                    .FirstOrDefault();
