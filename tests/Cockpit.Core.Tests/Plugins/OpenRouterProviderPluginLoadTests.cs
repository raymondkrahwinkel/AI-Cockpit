...
/// <summary>
/// End-to-end loader proof for the OpenRouter provider plugin (AC-806): loads the real compiled plugin
/// through <see cref="PluginActivator"/> and asserts it registers its session provider. Mirrors <see cref="GitHubModelsProviderPluginLoadTests"/>.
/// </summary>
public class OpenRouterProviderPluginLoadTests
{
    [Fact]
    public void ActivatesAndRegistersTheSessionProvider_WhenBuilt()
    {
        ...
        var discovered = new DiscoveredPlugin(folder, "openrouter-provider", manifest, hash, PluginLoadDecision.Load);
        ...
        Assert.Equal("openrouter-provider", plugin!.Metadata.Id);
        Assert.Equal("OpenRouter", plugin.Metadata.DisplayName);
        ...
        Assert.Contains(host.SessionProviders, registration => registration.ProviderId == "openrouter-provider.openrouter" && registration.DisplayName == "OpenRouter");
        ...
        Assert.Equal("Cockpit.Plugin.OpenRouterProvider.UI.dll", manifest.UiAssembly);
        Assert.Equal("Cockpit.Plugin.OpenRouterProvider.UI.OpenRouterProviderUi", manifest.UiEntryType);
        ...
        uiHost.Received(1).AddProviderConfigView("openrouter-provider.openrouter", Arg.Any<Func<string?, IPluginProviderConfigView>>());
    ...
    private static string? _LocatePluginOutput()
    {
        ...
        {
            var candidateRoot = Path.Combine(directory.FullName, "plugins-dev", "Cockpit.Plugin.OpenRouterProvider", "bin");
            if (Directory.Exists(candidateRoot))
            {
                var dll = Directory
                    .EnumerateFiles(candidateRoot, "Cockpit.Plugin.OpenRouterProvider.dll", SearchOption.AllDirectories)
                    .FirstOrDefault();
