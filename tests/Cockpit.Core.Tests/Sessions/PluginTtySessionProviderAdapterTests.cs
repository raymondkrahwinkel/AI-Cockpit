...
public class PluginTtySessionProviderAdapterTests
{
    ...
    // AC-218: the TTY route resolves its servers synchronously and had no session context at all, so it asked the
    // unscoped catalog. The project now travels on the launch context — deliberately as a value rather than a lookup,
    // because resolving a pane to a project means the UI thread, and this path cannot wait on it.
    [Fact]
    public void BuildLaunch_AsksForTheRegistryAsTheProjectSeesIt()
    {
        ...
        catalog.GetServersForProjectAsync(Arg.Any<string?>(), Arg.Any<CancellationToken>())
            .Returns(new List<Cockpit.Core.Mcp.McpServerConfig>());
        ...
        catalog.Received().GetServersForProjectAsync("project-1", Arg.Any<CancellationToken>());
    }

    [Fact]
    public void BuildLaunch_AServerTheProjectBringsItself_ReachesThePlugin()
    {
        var catalog = Substitute.For<Cockpit.Core.Abstractions.Mcp.IMcpServerCatalog>();
        catalog.GetServersForProjectAsync("project-1", Arg.Any<CancellationToken>()).Returns(new List<Cockpit.Core.Mcp.McpServerConfig>
        {
            new() { Name = "project-own", Transport = Cockpit.Core.Mcp.McpTransport.Http, Url = "http://127.0.0.1:3/mcp" },
        });
        ...
        {
            ProjectId = "project-1",
            EnabledMcpServerNames = new HashSet<string> { "project-own" },
        };
