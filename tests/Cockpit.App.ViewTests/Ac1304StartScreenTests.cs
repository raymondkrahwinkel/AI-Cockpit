...
public sealed class Ac1304StartScreenTests
{
    ...
    public void ThreeDoors_AllLandOnTheSameStartScreen()
    {
        ...
        // Door 1: a first start with nothing picked. The assistant is the conversation then, so this door opens
        // only where there is none — switch it off and the column has nothing left to draw.
        ...
        cockpit.AssistantChat = _AssistantChat(cockpit, new SessionViewModel { Title = "Assistant" });
    ...
    private static CockpitViewModel _SimpleStand(bool withAssistant)
    {
        ...
        cockpit.AssistantChat = _AssistantChat(cockpit, withAssistant ? new SessionViewModel { Title = "Assistant" } : null);
