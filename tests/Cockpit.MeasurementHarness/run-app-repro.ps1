param(
    [int]$Sessions = 2,
    [int]$StreamSeconds = 5,
    [ValidateSet("new-rows", "growing-tail")][string]$Shape = "new-rows",
...
try {
    ...
    [IO.File]::WriteAllText((Join-Path $work "cockpit-leaksim.trigger"), "apprepro:$Sessions,$StreamSeconds,$Shape")
    ...
    $done = Wait-ForJson (Join-Path $work "app-repro.done.json") ($StreamSeconds + 30) "stream completion"
