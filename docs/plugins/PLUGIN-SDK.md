...
| `logoAsset` | no | Either a bundled SVG's file name (e.g. `"depot.svg"`), tinted by category and drawn in place of `icon`, or a vendor's own `http(s)` logo URL, fetched live and shown in its own colours, uncropped — never redistributed as a file in this repo (AC-553; the three AI-provider plugins use this for their real vendor mark). A bare file name only resolves for plugins the *host itself* bundles the asset for — there is no download/cache path for a third-party store's own bundled-style logo name, so that shape of value is meaningless (and harmlessly ignored, falling back to `icon`) outside the official store. An `http(s)` URL works from any store: it is fetched the same way a store's own `iconUrl` is (capped at 1 MB / 8 s, silent on failure). |
