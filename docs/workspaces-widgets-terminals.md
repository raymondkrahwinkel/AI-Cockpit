...
**F0 — Pane abstraction (the foundational, careful refactor).** ✅ *The persisted model half landed (AC-410): an AI session is now a real `WorkspacePane` (`Kind: AiSession`) in `cockpit.json`, the same as a widget or a terminal pane — it survives a restart, comes back materialised without auto-starting, and offers to resume its earlier conversation (or degrades honestly when that is not possible). The rendering half is not done* — `SessionTilePanel`'s cell content is still session-specific, not yet generalised to an `IPane` (`AiSessionPane | TerminalPane | WidgetPane`); an `AiSessionPane` wrapping today's session VM with no behaviour change is still open.
