...
// (Fase 4). Mirrors the host's `ClaudeConfig(ConfigDir, ExecutablePath)` — the plugin cannot reference the
    ...
    [property: JsonPropertyName("executablePath")] string? ExecutablePath = null)
{
    ...
    // Reads a config JSON blob, or an empty config when it is blank/unreadable — a profile-less or default session.
