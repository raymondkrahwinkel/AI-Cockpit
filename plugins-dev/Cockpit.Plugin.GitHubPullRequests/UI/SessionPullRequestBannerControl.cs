...
internal sealed class SessionPullRequestBannerControl : UserControl
{
    ...
    public SessionPullRequestBannerControl(ICockpitUiHost host, IPluginSessionContext session)
    {
        _host = host;
        _session = session;
    ...
    private async Task _LoadAsync()
    {
        ...
        try
        {
            // AC-1396: the backend part runs the `gh pr view`; this only asks it over the channel.
            var payload = JsonSerializer.SerializeToElement(new SessionPullRequestRequest(directory), GitHubPullRequestsChannel.Json);
            var answer = await _host.Channel.InvokeAsync(GitHubPullRequestsChannel.SessionPullRequest, payload, cts.Token);
            status = answer.Deserialize<SessionPullRequestStatus>(GitHubPullRequestsChannel.Json);
        }
        catch (Exception)
        {
            status = null;
