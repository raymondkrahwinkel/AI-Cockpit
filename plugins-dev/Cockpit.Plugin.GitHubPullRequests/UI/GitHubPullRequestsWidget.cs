...
//
// AC-1396: the shared source lives in the backend part now; this reads it over the channel — its last snapshot on
// attach, every later one from the FeedUpdated event. The session-signal debounce this kept for itself is gone:
// the backend's PullRequestBadgeUpdater already refreshes the same shared source on that signal.
internal sealed class GitHubPullRequestsWidget : UserControl
{
    ...
    private IDisposable? _feedUpdated;

    public GitHubPullRequestsWidget(GitHubPullRequestsSettings settings, ICockpitUiHost host, IWidgetContext context)
    {
        _settings = settings;
        _host = host;
        _context = context;
        ...
        viewAll.Click += async (_, _) => await _host.ShowDialogAsync(
        ...
        // "Loading…" until the backend's last known answer arrives, which is never a wait for gh itself.
        _ApplyState(new PullRequestFeedState(PullRequestFeedSnapshot.Empty, null));
    }
    ...
    private void _OnFeedUpdated(PluginChannelEvent channelEvent)
    {
        if (channelEvent.Payload.Deserialize<PullRequestFeedState>(GitHubPullRequestsChannel.Json) is { } state)
        {
            Dispatcher.UIThread.Post(() => _ApplyState(state));
        }
    }
    ...
    protected override void OnAttachedToVisualTree(VisualTreeAttachmentEventArgs e)
    {
        ...
        _feedUpdated = _host.Channel.Subscribe(GitHubPullRequestsChannel.FeedUpdated, _OnFeedUpdated);
        _ShowCurrent();
    }
    ...
    protected override void OnDetachedFromVisualTree(VisualTreeAttachmentEventArgs e)
    {
        ...
        _feedUpdated?.Dispose();
        _feedUpdated = null;
    }

    // The same last-resort shape as a timer tick: an attach handler cannot await, so a failed ask is caught here.
    private async void _ShowCurrent()
    {
        try
        {
            var answer = await _host.Channel.InvokeAsync(GitHubPullRequestsChannel.Feed, GitHubPullRequestsChannel.NoPayload);
            if (answer.Deserialize<PullRequestFeedState>(GitHubPullRequestsChannel.Json) is { } state)
            {
                _ApplyState(state);
            }
        }
    ...
    // What the pane's ↻ does: ask the shared source for a forced refresh, and report this call's own failure.
    private async Task _RefreshAsync()
    {
        ...
        try
        {
            ...
            var answer = await _host.Channel.InvokeAsync(GitHubPullRequestsChannel.Refresh, payload);
            if (answer.Deserialize<PullRequestRefreshAnswer>(GitHubPullRequestsChannel.Json) is { Error: { } error })
            {
                _Say($"Could not load pull requests: {error}");
            }
        }
        catch (Exception exception)
        {
    ...
    private void _ApplyState(PullRequestFeedState state)
    {
        ...
        _stale.IsVisible = fetchedAt is { } at && DateTimeOffset.UtcNow - at > PullRequestFeedSnapshot.StaleAfter;
        ...
        {
            _loading.IsVisible = state.LastError is null;
            _Say(state.LastError is { } error ? $"Could not load pull requests: {error}" : "Loading pull requests…");
        }
    }
