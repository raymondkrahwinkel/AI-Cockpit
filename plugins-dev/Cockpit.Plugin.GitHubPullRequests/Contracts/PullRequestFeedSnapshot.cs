...
// `StaleAfter` itself, at render time, against the clock as it is then.
//
...
{
    // Three of GitHubPrGhClient.PullRequestTtl (AC-1396: spelled out here so the UI part, which cannot see that
    // client, can mark a snapshot old too). PullRequestRefreshSourceTests pins the two against each other.
    public static readonly TimeSpan StaleAfter = TimeSpan.FromMinutes(5) * 3;
