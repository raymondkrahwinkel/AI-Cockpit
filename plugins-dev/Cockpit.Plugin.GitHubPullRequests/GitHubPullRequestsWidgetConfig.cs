...
// A record round-tripped through the instance's `IPluginStorage` as one JSON value, so a half-written
...
{
    // Fewest and most a pane may show — the same 1–20 range the section's count is clamped to.
    public const int MinItems = 1;
    public const int MaxItemsAllowed = 20;

    // How many pull requests this pane lists. Defaults to ten — a dashboard pane is roomier than the side strip's five.
    ...
    // What a freshly placed widget shows before anyone opens its settings.
