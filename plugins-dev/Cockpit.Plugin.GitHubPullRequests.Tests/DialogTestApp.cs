...
/// </summary>
/// <remarks>
/// Loads the same styles, in the same order, as <c>src/Cockpit.App/App.axaml</c> (AC-338), the same way
/// <c>Cockpit.Plugin.GitHubIssues.Tests</c> does: Fluent, then the material icon set, then the DataGrid's own
/// Fluent theme, then the cockpit's <c>Theme.axaml</c> read off disk (the plugin cannot reference
/// <c>Cockpit.App</c>). Without the cockpit theme, every <c>_Brush("Cockpit…")</c> lookup in the view resolves to
/// nothing.
/// </remarks>
