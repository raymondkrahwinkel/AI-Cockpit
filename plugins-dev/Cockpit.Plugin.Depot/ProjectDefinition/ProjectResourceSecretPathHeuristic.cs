...
// `Cockpit.Core.Projects.ProjectResourceSecretPathHeuristic` exactly, the same "cannot reference Cockpit.Core"
// constraint `ProjectResourcePortabilityClassifier` already documents for itself (AC-244/AC-605). Kept
// in sync by `ProjectResourceSecretPathParityTests`, which runs the same table against both copies and goes
...
// `CockpitProjectResourceEntry.Create`) — a false positive here costs three things at once (reported,
