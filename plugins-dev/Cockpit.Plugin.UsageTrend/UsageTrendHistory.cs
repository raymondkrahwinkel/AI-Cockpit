...
// that keep this out of `cockpit.json`'s way — when a fresh reading is worth writing and when it is just the
// same story a moment later.
// - <description>
// *Debounce.* The session header updates every few seconds; writing each tick would rewrite the whole
// settings file hundreds of times an hour. So a reading is written at most once per `MinInterval` per
// profile — unless it jumped.
// </description>
// - <description>
// *Jump.* A sharp move — a `/compact` dropping the context, an allowance climbing past
// `JumpThresholdPercent` points — is written straight away, so a knife-edge is not flattened into the
// gap between two debounced points.
// </description>
// - <description>
// *Retention.* Nothing older than `RetentionDays` is kept — a trend widget answers "how did my
// week go", not "my year", and that ceiling is what keeps the payload in `cockpit.json` small.
// </description>
// All per profile: debounce and jump compare against the last sample for the *same* profile label, so one
// busy profile does not gate another's first point.
internal static class UsageTrendHistory
{
    // How long history is kept — 14 days, the AC-54 retention (§3): enough for "how did my week go", small enough to stay tens of KB in the settings file.
    public const int RetentionDays = 14;

    // The most a routine reading is written: once per 10 minutes per profile. A jump overrides it.
    public static readonly TimeSpan MinInterval = TimeSpan.FromMinutes(10);

    // A move of at least this many percentage points on any metric is written at once, debounce or not — a scarp the 10-minute grid would otherwise skip.
    public const double JumpThresholdPercent = 10.0;

    // Decides what the history becomes when `candidate` is offered, and returns the new list —
    // or `null` when nothing changes, so the caller can skip the write entirely (the debounce that
    // keeps `cockpit.json` quiet). A candidate with no usage figure at all is never recorded.
    ...
    // Drops every sample older than `RetentionDays` before `now`, keeping the rest in order. Public for the widget's load path, which prunes what it read before charting it.
    ...
    // Whether `candidate` is worth writing given the last sample for its profile: the first one always, one past the debounce window always, and one that jumped even inside the window.
    ...
    // jump in itself — a context reset after `/compact` reads as a value going to null, and a subscription's
