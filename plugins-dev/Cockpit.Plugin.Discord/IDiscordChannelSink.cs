...
{
    /// <summary>Posts a new message, optionally with Approve/Deny buttons for <paramref name="consentPromptId"/>. Returns the new message's id.</summary>
    Task<ulong> PostAsync(string text, Guid? consentPromptId = null, CancellationToken cancellationToken = default);

    /// <summary>Edits a message already returned by <see cref="PostAsync"/>. <paramref name="keepButtons"/> false strips any consent buttons.</summary>
    Task EditAsync(ulong messageId, string text, bool keepButtons = true, CancellationToken cancellationToken = default);

    /// <summary>Adds a reaction emoji to an inbound message — the plugin's only reply to a sender-visible failure.</summary>
