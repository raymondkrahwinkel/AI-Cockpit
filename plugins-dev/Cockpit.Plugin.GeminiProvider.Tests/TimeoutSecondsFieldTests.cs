...
// TryGetConfigJson's JSON. Exercises TimeoutSecondsField directly, the seam the view itself calls.
public class TimeoutSecondsFieldTests
{
    [Theory]
    ...
    public void TryParse_AcceptsBlankOrPositive_RejectsZeroOrNegative(string text, bool expectedOk, int? expectedValue)
    {
        var ok = TimeoutSecondsField.TryParse(text, out var timeoutSeconds);
