...
{
    // The widest a row gets before the layout adds another row: past three, a session pane is too narrow to read.
    ...
    // How many tiles each row holds: as few rows as `MaxTilesPerRow` allows, then the tiles spread
