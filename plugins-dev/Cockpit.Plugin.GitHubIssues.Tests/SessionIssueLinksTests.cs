...
public class SessionIssueLinksTests
{
    ...
    [Fact]
    public async Task Link_SetsTheStatuslineOfThePaneItLinksTo()
    {
        var host = new FakeCockpitHost();
        var links = new SessionIssueLinks(host);

        await links.LinkAsync("pane-1", Issue);

        Assert.Equal("hello-world#42 — Link to session leaves the name alone", host.Statuslines["pane-1"]);
    }

    [Fact]
    public async Task Link_SuggestsTheIssueAsTheName_RatherThanTakingIt()
    {
        var host = new FakeCockpitHost();
        var links = new SessionIssueLinks(host);
    ...
    [Fact]
    public async Task Link_WithNoPane_LabelsNothing()
    {
        var host = new FakeCockpitHost();
        var links = new SessionIssueLinks(host);

        await links.LinkAsync(string.Empty, Issue);
    ...
    [Fact]
    public async Task Unlink_LeavesTheStatuslineAlone()
    {
        var host = new FakeCockpitHost();
        var links = new SessionIssueLinks(host);
        await links.LinkAsync("pane-1", Issue);
