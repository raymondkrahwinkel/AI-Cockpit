...
// link the project editor already stores (AC-317) in the session's environment as `GH_REPO` — the variable
// `gh` reads to decide which repository a command without an explicit `--repo` is about.
//
// Without this the link only reached this plugin's own dialogs: an agent shelling out to `gh` inside the
...
{
    // What `gh` reads as "the repository this command is about" when none is given on the command line.
