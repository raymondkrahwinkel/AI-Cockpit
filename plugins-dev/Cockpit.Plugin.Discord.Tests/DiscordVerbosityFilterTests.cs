...
public class DiscordVerbosityFilterTests
{
    [Theory]
    ...
    public void FinalAnswerOnly_RelaysOnlyTheAnswerAndErrors(AssistantChannelRowKind kind, bool expected) =>
        Assert.Equal(expected, DiscordVerbosityFilter.ShouldRelay(kind, AssistantChannelVerbosity.FinalAnswerOnly));
    ...
    public void Everything_RelaysEverythingButTheDivider(AssistantChannelRowKind kind, bool expected) =>
        Assert.Equal(expected, DiscordVerbosityFilter.ShouldRelay(kind, AssistantChannelVerbosity.Everything));

    [Theory]
    [InlineData(AssistantChannelRowKind.ToolUse, true)]
    [InlineData(AssistantChannelRowKind.Divider, false)]
    public void StatusLines_RelaysTheSameSetAsEverything(AssistantChannelRowKind kind, bool expected) =>
        Assert.Equal(expected, DiscordVerbosityFilter.ShouldRelay(kind, AssistantChannelVerbosity.StatusLines));
    ...
    public void Everything_RendersRowTextVerbatim()
    {
        ...
        Assert.Equal("the full tool call text", DiscordVerbosityFilter.Render(row, AssistantChannelVerbosity.Everything));
    }
    ...
    public void StatusLines_CollapsesToolUseIntoAShortLine()
    {
        ...
        var rendered = DiscordVerbosityFilter.Render(row, AssistantChannelVerbosity.StatusLines);
    ...
    public void StatusLines_LeavesAssistantTextFullLength()
    {
        ...
        Assert.Equal("the assistant's actual answer", DiscordVerbosityFilter.Render(row, AssistantChannelVerbosity.StatusLines));
    }
