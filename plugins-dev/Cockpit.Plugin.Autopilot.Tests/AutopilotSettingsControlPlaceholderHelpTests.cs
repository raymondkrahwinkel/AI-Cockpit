...
/// <summary>
/// AC-521: a placeholder documented in the Templates section's help that <see cref="AutopilotTemplateResolver.Resolve"/>
...
/// actual rendered help text, and the "replaced" set is parsed out of <see cref="AutopilotTemplateResolver"/>'s own
/// source — its <c>IssueDataKeys</c> dictionary for the fixed <c>{{issue.*}}</c> tokens, and its own
/// <c>token.StartsWith("...")</c> check for the <c>input.*</c> prefix — a hardcoded list here could carry the same
/// mistake the production code does.
/// <para>
/// <c>{{input.&lt;name&gt;}}</c> is not a fixed token like the others — it names a whole class of operator-supplied
...
/// the resolver's own prefix literal ("input.") is extracted from its <c>StartsWith</c> call and turned into the
