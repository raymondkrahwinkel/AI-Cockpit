...
internal sealed class FakeCliSubprocess : ICliSubprocess
{
    ...
    // P1-13: a plain List<string> here throws an intermittent InvalidOperationException ("Collection was
    // modified") once a background write races a test's own LINQ read over it — harmless for the Codex
    // template this was copied from, but not here: the AC-274 usage poll writes unsolicited traffic right after
    // every turn, on its own task, concurrently with whatever a test is asserting. ConcurrentQueue's enumerator
    // is documented safe to use while another thread concurrently enqueues.
