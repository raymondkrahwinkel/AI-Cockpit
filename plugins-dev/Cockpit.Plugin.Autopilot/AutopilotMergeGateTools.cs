...
internal sealed class AutopilotMergeGateTools(ICockpitHost host, AutopilotRunManager manager)
{
    ...
    [Description("Give or refuse the go for an Autopilot run waiting at its merge gate — the sub named by `issue` has passed both review gates and its evidence package (pull request, HEAD, diff-stat against the epic's collection branch, gate verdicts, build) was handed to cockpit-assistant. go=true rebases the run branch onto the collection branch, merges its pull request, builds the new tip and reports the exit code on the epic; go=false leaves the branch and its pull request untouched and records your reason on the epic. Read the diff-stat first: a deletion of files another sub added is the failure this gate exists to catch. Any session this endpoint is mounted in may answer — the epic comment records which pane did — and it is refused when no run is waiting at the gate for that issue. Name the epic itself (e.g. \"AC-343\") to answer its end gate instead: once every sub is merged and the gate's report (full suite on the pinned tip, red set against the baseline run on main, test files and counts per sub) was handed to you, go=true opens the pull request from the collection branch to main and go=false leaves the branch as it is.")]
