...
internal static class AutopilotRunReliability
{
    ...
    // Whether a settled run counts as clean: it reached MergeReady, no step needed a correction, and it actually
    // delivered its pull request. A merge-ready run that could not open its PR still needs a human to do so by
    // hand, which is exactly the intervention "clean" rules out.
