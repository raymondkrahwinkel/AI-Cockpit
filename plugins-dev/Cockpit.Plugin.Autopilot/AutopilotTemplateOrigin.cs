...
// Where an `AutopilotTemplate` came from (AC-189), which decides whether the operator may edit or delete
// it. A `Builtin` ships with the Autopilot plugin; a `Plugin` is contributed by another
// plugin at run time; a `User` template was authored by the operator. Builtin and Plugin templates can be
