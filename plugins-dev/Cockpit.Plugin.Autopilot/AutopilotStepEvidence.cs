...
/// a closer look. This is what the CEO validates against instead of re-reading the whole worktree itself. The step
/// agent neither writes it nor can influence it: it is composed here, from what
/// <see cref="IAutopilotEvidenceSource"/> saw, and the agent's own summary is carried separately in the validation turn
/// so the two can never be mistaken for each other.
/// </summary>
/// <param name="Observation">What the harness saw, already worded for the validation turn.</param>
/// <param name="Concerns">
/// What the harness flagged about the change (<see cref="AutopilotEvidenceSignals"/>). Empty means no spot-check fired
