...
// asks for. A CEO-first run has *no* source (this is null): Autopilot is started with only the CEO and the goal
...
internal sealed record AutopilotPlanSource(string Tracker, string IssueId, string Title, string Description = "", string Url = "", string EpicId = "")
{
    ...
    // full item and a template's `{{issue.url}}` resolves to the real link instead of blank (AC-189). Carries
    // `AutopilotRun.EpicId` through unchanged (AC-346) — empty for a run clicked directly on its own item.
