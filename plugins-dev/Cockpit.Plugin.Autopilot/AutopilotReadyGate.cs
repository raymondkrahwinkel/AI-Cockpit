...
/// <summary>Whether Autopilot may start an item, and — when it may not — the sentence the operator and the issue get told.</summary>
...
internal static class AutopilotReadyGate
{
    ...
    /// <summary>
    /// Decides on <paramref name="reportedStages"/> — the stages the tracker says the item is on, one per line (a
    /// YouTrack issue has a single stage, a GitHub issue has its labels) — against the stage the operator configured as
    /// executable for that tracker. A blank <paramref name="executableStage"/> turns the gate off deliberately; an item
