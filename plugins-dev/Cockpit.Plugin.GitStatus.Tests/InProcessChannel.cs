...
// Both halves of the plugin's channel in one object (AC-1390): what the backend part handles is what the UI part
// invokes, as the host's PluginChannelHub does for one plugin. GitStatus publishes nothing, so events go nowhere.
internal sealed class InProcessChannel : IPluginBackendChannel, IPluginUiChannel
{
    ...
    public IReadOnlyCollection<string> Actions => _handlers.Keys;
    ...
    public Task<JsonElement> InvokeAsync(string action, JsonElement payload, CancellationToken cancellationToken = default) =>
        _handlers.TryGetValue(action, out var handler)
            ...
            : throw new PluginChannelUnknownActionException("git-status", action);
