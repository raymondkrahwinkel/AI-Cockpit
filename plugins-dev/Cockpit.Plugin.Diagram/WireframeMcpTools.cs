...
internal sealed class WireframeMcpTools(ICockpitHost host, IWireframeAccessRegistry registry, DiagramSettings settings)
{
    ...
    [Description("Sets or clears ONE modifier on ONE component and applies it straight away, leaving everything else alone — the way to make a button primary, tick a checkbox, size a column, fill in a value, lay a flow to another screen or attach a requirement the format cannot draw, without rebuilding the component. `modifier` is one of: primary, selected, checked, disabled, w, h, align, value, goto, note. For the four flags (primary/selected/checked/disabled) omit `value` to turn it on, or pass `clear: true` to take it off. For w/h (a flex ratio 1-6, never pixels), align (left/center/right), value (text for most components, 0-100 for slider/progress/pagination), goto (a screen's title, from read_wireframe's `screens`) and note (a free-text requirement, e.g. \"disabled until both fields are filled in\") pass the new `value`, or `clear: true` to remove it. Refused with a reason if there is no component with that id any more, the modifier is not one this format has, it has no meaning on this component here (e.g. `w:` on something that is not a row/header/footer child), or the operator is editing it right now.")]
    public Task<string> SetComponentModifier(
        [Description("Your session id (COCKPIT_PANE_ID).")] string session,
        [Description("The wireframe to edit, by its id or name from list_wireframes.")] string wireframe,
        [Description("The id of the component to change.")] string component,
        [Description("The modifier keyword: primary, selected, checked, disabled, w, h, align, value, goto or note.")] string modifier,
        [Description("The value to set, e.g. `2` for w/h, `right` for align, `Raymond` for value, a screen's title for goto, a requirement sentence for note. Leave empty for a flag modifier you are turning on.")] string? value = null,
        [Description("True to remove the modifier instead of setting it.")] bool clear = false)
    {
        ...
        {
            return Task.FromResult(_Serialize(new { ok = false, error = $"\"{modifier}\" is not a modifier this format has — use one of: primary, selected, checked, disabled, w, h, align, value, goto, note." }));
        }
        ...
        // AC-907: a note is a sentence, unconditionally quoted for the same reason — including the rare one that is
        // all digits, which int.TryParse would otherwise wave through unquoted.
        var quoted = name is WireframeModifierName.Goto or WireframeModifierName.Note
            || (name == WireframeModifierName.Value && !int.TryParse(value, out _));
        var edit = isFlag
    ...
    private static void _Collect(WireframeNode node, WireframeNode screen, IReadOnlyList<WireframeNode> screens, int depth, List<object> into)
    {
        ...
        into.Add(new
        {
            ...
            @goto = target,
            // AC-907: verbatim, unlike goto — there is nothing to resolve, the value is the whole answer.
            note = node.ValueOf(WireframeModifierName.Note),
        });
