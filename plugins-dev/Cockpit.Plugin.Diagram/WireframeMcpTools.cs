...
internal sealed class WireframeMcpTools(ICockpitHost host, IWireframeAccessRegistry registry, DiagramSettings settings)
{
    ...
    public Task<string> SetComponentModifier(
        ...
        [Description("The modifier keyword: primary, selected, checked, disabled, w, h, align, value, goto, note or replaces.")] string modifier,
        [Description("The value to set, e.g. `2` for w/h, `right` for align, `Raymond` for value, a screen's title for goto, a requirement sentence for note, or `#results` for replaces. Leave empty for a flag modifier you are turning on.")] string? value = null,
