...
// AC-816: the quick-start's one screen — name (prefilled, Enter is enough) and, optionally, the active session
// to couple. No wizard, no kind field (whiteboard is its own plugin since AC-820).
internal static class DiagramQuickStartDialog
{
    public static async Task<DiagramQuickStart?> ShowAsync(ICockpitHost host, string defaultName)
    {
        DiagramQuickStart? result = null;

        await host.ShowDialogAsync("Nieuw diagram", () =>
        {
            ...
            void Confirm()
            {
                ...
                result = new DiagramQuickStart(name, coupleSession.IsChecked == true ? activePaneId : null);
            }
        ...
        }, "diagram.quickstart", width: 420, height: 240);
