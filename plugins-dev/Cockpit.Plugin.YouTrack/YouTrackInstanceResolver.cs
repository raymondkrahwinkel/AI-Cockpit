...
// Picks the YouTrack instance an attach targets (AC-116).
internal static class YouTrackInstanceResolver
{
    // The instance to attach to: matched by the issue URL's `host` when the tool result
    ...
    // host only ever *selects* among configured instances — the request always goes to the matched
