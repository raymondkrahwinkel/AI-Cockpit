...
//
// A *state-machine* project has a real transition graph, and YouTrack hands it over per issue
// (`YouTrackStateField.PossibleEvents`): those, and only those, are the moves. Nothing here needs to be
// invented — the board already said.
//
// An *ordinary* status field has no graph at all. YouTrack will let you put a ticket from Backlog straight to
...
internal static class StateFlow
{
    ...
    // The previous column: putting something back. Null at the start of the board.
