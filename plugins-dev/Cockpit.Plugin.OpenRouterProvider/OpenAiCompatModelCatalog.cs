...
// AC-930: the models a profile's own base URL currently offers, over the OpenAI-compatible `GET /models`.
// The base URL already carries the provider's version segment, so only the resource is appended here —
// unlike the host's local-server catalog, which inserts `/v1` itself.
