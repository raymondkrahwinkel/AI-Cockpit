...
// AC-806: this plugin's own provider config, never seen by the host — only (de)serialized here via the
// opaque `ConfigJson` the host round-trips. `Model` carries OpenRouter's `vendor/model` notation verbatim.
