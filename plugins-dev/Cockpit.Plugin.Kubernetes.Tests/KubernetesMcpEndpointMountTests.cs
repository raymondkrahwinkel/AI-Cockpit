...
public class KubernetesMcpEndpointMountTests
{
    [Fact]
    public void Initialize_MountsTheKubernetesMcpEndpoint()
    {
        var host = Substitute.For<ICockpitHost>();
        host.Storage.Returns(new FakePluginStorage());

        using var plugin = new KubernetesPlugin();
        plugin.Initialize(host);

        _ = host.Received(1).AddMcpEndpoint("cockpit-k8s", Arg.Any<object>(), Arg.Any<Func<bool>?>());
    }
}
