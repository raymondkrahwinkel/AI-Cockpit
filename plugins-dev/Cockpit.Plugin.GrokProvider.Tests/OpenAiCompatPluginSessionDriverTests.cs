...
// `OpenAiCompatPluginSessionDriver` against a fake `IChatClient` (AC-724, mirroring
// the OpenRouter provider plugin's AC-806 `OpenAiCompatPluginSessionDriverTests`) — same
...
public class OpenAiCompatPluginSessionDriverTests
{
    ...
    public async Task SendUserMessage_StreamsAssistantDeltas_ThenCompletesTheTurn()
    {
        ...
        var driver = new OpenAiCompatPluginSessionDriver(chatClient, "grok-4.6");
    ...
    public async Task StartAsync_WithAModelOverride_UsesItForTheTurnInsteadOfTheDefault()
    {
        ...
        await driver.StartAsync(model: "grok-4-fast");
        await driver.SendUserMessageAsync("hi");
        await _CollectUntilTurnCompletedAsync(driver);

        Assert.NotNull(captured);
        Assert.Equal("grok-4-fast", captured!.ModelId);
    }
