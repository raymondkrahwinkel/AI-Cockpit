...
// The switch: one value, a way out per case — `{state}` against "In Progress, Review, Done". Where the If has
...
// an error — it is the case the operator did not name, and the pin is there to catch exactly that. A *missing*
// value is different: `{state}` when nothing called state came in is a step configured against data that is not
