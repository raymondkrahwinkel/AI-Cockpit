...
// One step on the canvas (#69): an instance of a `NodeTypeDescriptor`, with its own name, its place
// and its parameter values. What it *does* lives in the type, not here — which is what lets a plugin one
// day contribute a type this editor has never heard of and have it draw and run like any other.
public sealed class WorkflowNode
{
    public required string Id { get; init; }

    // The type this is an instance of ("cockpit.notify").
    public required string TypeId { get; init; }

    // What the operator called it. Three "Notify" nodes in one flow are otherwise indistinguishable.
    ...
    // The values for this type's parameters, by name. Opaque here on purpose: the editor never needs to know what a node type stores.
    public Dictionary<string, string> Parameters { get; init; } = [];

    // A node the operator switched off: it stays on the canvas, drawn dimmed, and a run skips it. Deleting is not the only way to say "not now".
    public bool IsDisabled { get; set; }

    // Print what this step hands on into the run log, in full. The debug switch: what a step produced is the thing you need when a flow does the wrong thing, and a one-line summary is not it.
    ...
    //
    // An *error wire* (see `ErrorOutput`) says more and says it better: it sends the failure
    ...
    //
    // Never written to the file: it is looked up from `TypeId`, so storing it would be storing the same
    ...
    // the operator wrote into it (see `SwitchCases`), so they are read from this node's own parameters.
    [JsonIgnore]
