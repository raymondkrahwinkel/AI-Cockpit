...
//
// There is always an `Otherwise`, and it is always last. A value that matches none of the cases has to
...
public static class SwitchCases
{
    ...
    // The way out taken by a value that matched no case.
    public const string Otherwise = "otherwise";

    // The parameter holding the value to match — an expression over the run's data, e.g. `{state}`.
    public const string ValueParameter = "Value";

    // The parameter holding the cases: comma- or newline-separated, in the order their pins appear.
    ...
    // What this switch's pins are called: a case each, then "otherwise".
