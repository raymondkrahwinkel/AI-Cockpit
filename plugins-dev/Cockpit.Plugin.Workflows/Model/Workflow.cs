...
public sealed class Workflow
{
    ...
    // When it was last changed — what the manager sorts and shows, so "which one was I working on" has an answer.
    ...
    // follow: a node feeding itself, anything running *into* a trigger, and a wire that is already there.
    ...
    // Adds the wire when the rules allow it, and says why when they do not.
    ...
    // Removes a node and every wire that touched it — a wire to a step that is gone is not a wire.
    ...
    // do (its pins are the cases written into it, see `SwitchCases`). A wire remembers its way out by
    // *position*, so inserting a case in front of the others would silently hand every wire below it to the
    ...
    //
    // `nodeId`: The node whose ways out changed.
    // `before`: What its ways out were called before the change, in their old order.
    // The labels of the wires that had nowhere left to go, in the order they were dropped.
    ...
    // Whether this way out already leads somewhere — what decides if the canvas draws the "+" that adds the next step.
