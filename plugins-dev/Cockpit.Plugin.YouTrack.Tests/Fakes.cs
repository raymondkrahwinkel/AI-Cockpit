...
// An in-memory `IPluginStorage` for exercising `YouTrackSettings` without the host's real per-plugin store.
...
internal sealed class FakeCockpitHost : ICockpitHost
{
    ...
    // What the last New-session dialog was asked to open with — the fields the plugin filled in for the operator.
    public NewSessionPrefill? LastNewSessionPrefill { get; private set; }
    ...
    // AC-884: `ProjectFieldValues` stores the raw comma-separated value exactly as the real host's project store
    // would — split here the same simple way the host's own `ProjectLinkValues.Split` does, so a test that sets
    // "EWB, AT" sees both prefixes back.
