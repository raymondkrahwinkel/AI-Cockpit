...
// What one local run is asked to do: a job, in a workflow, in a checkout.
//
// `ProjectRoot`: The session's worktree — the code the run is about, not a fresh clone.
// `WorkflowPath`: Absolute path to the workflow file, as `WorkflowCatalog` produced it.
...
//
// `RunnerImage`: The image act runs the job in. Named explicitly on every run — see `ActCommand`.
// `CpuLimit`: How many cores the job container may use.
internal sealed record ActRunOptions(string RunnerImage, int CpuLimit)
{
    // act's own medium image: the tools an action needs to bootstrap, without the 50 GB of the large one.
    public const string DefaultRunnerImage = "catthehacker/ubuntu:act-latest";

    // The package cache that survives between runs — the whole reason a second run is faster than the first.
    public const string NugetVolume = "cockpit-local-ci-nuget";

    // The SDK the setup step installs, kept out of the container's lifetime for the same reason.
    ...
    // Stamped on every container of a run, so stopping one can find what it left behind.
