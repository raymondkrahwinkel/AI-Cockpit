...
The screenshot tool's own selection surface has scenes too — `screenshot-selection` for the resting surface,
and `-region`, `-window` and `-redaction` for the states it is dragged, keyed and clicked into. It covers the
whole screen and takes every key, so it is the one window that cannot be looked at by running the app.
