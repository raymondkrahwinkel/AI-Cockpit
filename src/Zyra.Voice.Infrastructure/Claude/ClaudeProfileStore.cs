...
/// <c>zyra-voice.json</c> in the app's config directory (<c>%APPDATA%\ZyraVoice</c> on
...
internal sealed class ClaudeProfileStore : IClaudeProfileStore, ISingletonService
{
    ...
    public ClaudeProfileStore()
        : this(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "ZyraVoice", "zyra-voice.json"))
    {
    }
