...
/// survives as the inner exception of the <c>HttpRequestException</c> the caller sees. A pin mismatch and a wrong
/// bearer token are different events — one is a credential the operator can fix, the other is the machine at that
/// address not being the machine they paired with — and a transport failure that only said "TLS error" would put
/// them in the same bucket. That is the distinction criterion 8 asks for.
/// </remarks>
