...
// One backend's measured result (AC-68): how long the calibration clip took on it, and how much the
// desktop hitched while it ran. Latency is measured in an isolated child process (one native per process, so
// each backend needs its own), the hitch in the desktop process that was running alongside it.
public sealed record BackendMeasurement(VoiceBackendPreference Backend, double LatencyMs, double HitchMs);

// One model's measured latency on the chosen backend (AC-68): the model-ladder timing that backs the
...
// A step in a running calibration (AC-68): the line to show and, when a download can report one, a 0..1
// fraction for a bar. Null fraction means the step has no honest percentage (loading, warming up, measuring).
...
public static class TranscriptionCalibrationReport
{
    ...
    // The CPU is preferred while it is at most this many times slower than the GPU and the GPU is smooth.
    public const double CpuPreferenceFactor = 1.5;

    // A wider margin used when the GPU hitches the desktop: the CPU is preferred until it is this many
    // times slower, because the GPU's speed then comes at a visible cost.
    ...
    // The verdict's words for an already-measured calibration.
    public static string Rationale(TranscriptionCalibration calibration) => Decide(calibration.Measurements).Rationale;

    // A sentence should transcribe within this on the chosen backend to feel responsive for dictation.
    public const double ResponsiveModelBudgetMs = 3000.0;

    // Curated models from least to most accurate. Drives the "most accurate model that is still
    // responsive" pick; a name not on the list ranks lowest so an unmeasurable custom name can never be
    // recommended over a real measured model.
    ...
    // transcribes within `ResponsiveModelBudgetMs`. If nothing is responsive (a slow CPU-only box), the
