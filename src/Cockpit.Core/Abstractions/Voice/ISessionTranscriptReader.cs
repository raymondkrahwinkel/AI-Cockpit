...
/// <summary>
/// Tails a live TTY session's transcript for its status (#39): TTY mode runs the real interactive TUI, so there is
/// no parsed event stream to read from (unlike SDK mode) — but a provider's CLI writes the session to disk, so
/// tailing that record gets the turn's activity cleanly without touching the ANSI/TUI stream. The host owns neither
/// the location nor the format: this is a generic façade keyed by <see cref="SessionProfile"/> that dispatches to the
/// profile's provider plugin (which resolves and reads its own transcript), so the core knows no provider's transcript vocabulary.
/// </summary>
public interface ISessionTranscriptReader
{
    ...
    /// <paramref name="knownTranscriptsAtLaunch"/> (the session's own new record), then classifies each appended line
    /// into a coarse <see cref="SessionActivity"/> for the TTY status dot, dispatching to the profile's provider
    /// (which owns the format-specific reading) and also carrying the raw line so the host's output-signal observe
    /// surface reads from the same tail. A provider recording background work apart from the main transcript reports
    /// <see cref="SessionActivity.BackgroundBusy"/> while it runs, so a long background run never reads as done.
    /// Starts tailing from the record's current end — never the session's prior history — until <paramref name="cancellationToken"/>
    /// cancels, or ends immediately when the provider records nothing.
    /// </summary>
    ...
    /// oldest first, with the total the transcript holds. Unlike an SDK session, whose transcript the host builds and
    /// keeps as it streams, a TTY session's only record is the one its CLI writes to disk, so this is how the read
    /// surfaces (the assistant's <c>read_transcript</c>) answer for one at all. Empty when the provider records
    /// nothing, cannot name this session's transcript (the same <paramref name="statusFile"/> the tail is keyed on),
    /// or the session has yet to write anything — empty is the honest answer to all three rather than someone else's conversation.
    /// </summary>
