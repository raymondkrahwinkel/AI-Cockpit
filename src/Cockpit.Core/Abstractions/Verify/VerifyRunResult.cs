...
//
// `ExitCode`: The process exit code; -1 when it was killed on timeout.
// `StandardOutput`: Everything the command wrote to stdout.
// `StandardError`: Everything the command wrote to stderr.
// `Duration`: How long the command ran before exiting or being killed.
// `TimedOut`: True when the command was killed for exceeding the timeout.
