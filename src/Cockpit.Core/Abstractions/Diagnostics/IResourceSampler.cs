...
{
    /// <summary>
    /// The cockpit's own process.
    /// </summary>
    ResourceSample SampleSelf();

    /// <summary>
    /// The process with this id <em>and its descendants</em>. Returns <see cref="ResourceSample.None"/> when it is gone — a session that just exited is not an error.
    /// </summary>
    ResourceSample SampleTree(int processId);

    /// <summary>
    /// Whether this platform can actually see a process's descendants. False means the numbers cover only the process itself, and the UI must say so rather than quietly under-reporting.
    /// </summary>
