...
/// the four outcomes it landed on (<see cref="PluginProvisionOutcome"/>), without any UI of its own. Wraps <see cref="IPluginStoreClient"/> and <see cref="IPluginInstaller"/> — both already UI-free — so this is the one place their glue lives; the store dialog's install/update/rollback commands and any future UI-less installer (AC-541) call the same instance.
/// </summary>
