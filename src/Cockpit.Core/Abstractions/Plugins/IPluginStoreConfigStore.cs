...
/// <summary>
/// Persists the configured plugin stores (#14, AC-7) — the <c>pluginStores</c> list in <c>cockpit.json</c> the manager browses, each a remote (public or private) or a local folder.
/// </summary>
public interface IPluginStoreConfigStore
{
    ...
    /// <summary>
    /// Adds a store if one with the same kind and location is not already present (case-insensitive); replaces an existing one at the same location so its token can be updated.
    /// </summary>
