...
public sealed record AgentInboxTurnNotice(string PaneId, IReadOnlyList<AgentMessage> Messages, int Remaining)
{
    ...
    /// part of this notice that may differ by route, because it is the one part that <em>is</em> different — see
    /// <see cref="ArrivedOnAToolResult"/>. Everything after it, the trust statement above all, is shared: a recipient
    ...
    /// </summary>
    /// <param name="arrival">
    /// The opening clause, saying how these messages reached the recipient — <see cref="ArrivedWithThisTurn"/> by
    /// default, or <see cref="ArrivedOnAToolResult"/> for the piggyback route (AC-527). Only this clause varies:
