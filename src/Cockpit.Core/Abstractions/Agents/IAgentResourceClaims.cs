...
/// <c>list_claims</c> use — stops the AC-119 collision of two agents on one worktree. Advisory only, no OS lock.
/// Partitioned per workspace via caller-passed <c>workspacePaneIds</c>, not a workspace id, since a pane's derived
/// workspace drifts but its identity does not; cross-workspace collisions are the accepted gap, surfaced to the operator (AC-439).
/// </summary>
public interface IAgentResourceClaims
{
    ...
    /// it. Resources matched exactly, as callers wrote them. <paramref name="workspacePaneIds"/> is the host's own
    /// desk answer, always including <paramref name="paneId"/>; resolved before the lock is taken, so one claim per
    /// resource per desk is upheld, not guaranteed — a pane joining the desk mid-call can leave two visible claims,
    /// the newer releasable since <see cref="Release"/> looks for the caller's own claim first.
    /// </summary>
    AgentClaimResult Claim(string paneId, string resource, IReadOnlySet<string> workspacePaneIds);

    /// <summary>
    /// Gives up <paramref name="paneId"/>'s claim on <paramref name="resource"/>. Only the holder may — a release
    /// any neighbour could call guarantees nothing, since a mid-rebase agent's warning would quietly vanish. The
    /// caller's own claim is looked for first, so it can always release what it holds even in the rare case where a
    /// desk shows two claims on one name (see <see cref="Claim"/>).
    /// </summary>
...
/// deliberately never offers (AC-439), since an agent must never learn a claim exists on another desk. Exists for
/// the cross-workspace collision monitor, which tells the <em>operator</em>, not any agent, of a collision. Kept as
/// a separate interface, not a method on <see cref="IAgentResourceClaims"/>, so the boundary is enforced by which
/// interface a class is given: <c>AgentsMcpTools</c> is constructed with <see cref="IAgentResourceClaims"/> alone.
/// </summary>
