...
public static class SecretKey
{
    ...
    // OWASP's floor for PBKDF2-HMAC-SHA512 at the time of writing. Recorded per config, so raising it later does not strand an existing one.
    ...
    // A fresh random salt. One per installation, so the same password on two machines yields different keys.
    public static byte[] NewSalt() => RandomNumberGenerator.GetBytes(SaltBytes);

    // Derives the AES-256 key from the password.
