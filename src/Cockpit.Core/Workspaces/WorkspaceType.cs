...
/// <summary>
/// What a workspace hosts. Two host types ship built in — <see cref="Sessions"/> and <see cref="Dashboard"/> —
/// and a plugin can register its own (<c>ICockpitHost.AddWorkspaceType</c>), each identified by a stable,
...
/// </summary>
/// <remarks>
/// A value over an enum so the set is open: the host cannot enumerate the types a plugin will bring. The two
/// host <see cref="Id"/>s are the same strings the enum used to serialize to (<c>"Sessions"</c>,
/// <c>"Dashboard"</c>), so a <c>cockpit.json</c> written before this change loads unchanged. Use
/// <see cref="FromId"/> when reading an id from disk so the two host types keep matching case-insensitively as
...
public readonly record struct WorkspaceType(string Id)
{
    ...
    /// <summary>Whether this is one of the two built-in host types rather than a plugin-registered one.</summary>
    public bool IsBuiltIn => this == Sessions || this == Dashboard;

    /// <summary>
    /// The type for <paramref name="id"/>: one of the two host types when it names one (case-insensitively, as
    ...
    public static WorkspaceType FromId(string? id)
    {
        ...
        return string.Equals(id, Dashboard.Id, StringComparison.OrdinalIgnoreCase) ? Dashboard : new WorkspaceType(id);
    }
}
