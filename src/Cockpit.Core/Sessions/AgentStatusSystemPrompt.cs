...
// current (AC-544). Tool descriptions tell a model *what* the cockpit-session tools do; they say nothing
// about *when* reaching for them is worth it. `set_status`'s own description already says "update it
...
// Not the voice assistant's own prompt (`Cockpit.Core.Assistant.AssistantSystemPrompt`) — that one is
...
// something says so.
//
// *Not a delegated sub-agent, and that is a gap rather than a decision.* `DelegationService` builds its
...
// AC-142's). It also has no `SessionPanelViewModel` and therefore no statusline to keep, so the instruction
...
{
    // The default instruction. `NewSessionResult` puts it on every launch it composes, after a profile's own
    // `SystemPrompt` (AC-142) when there is one rather than instead of it — the arrangement AC-544's criterion
    // 5 points at by name. The delegation instruction works exactly this way already: it travels as its own value
    // and `ClaudeTtyProvider._AppendedInstructions` joins it to whatever the profile resolved, so a profile
