...
/// <see cref="SessionProfile"/> for each one that actually exists, labelled from its directory
...
/// </summary>
public static class ClaudeCliProfileDetector
{
    /// <summary>
    ...
    /// </summary>
    public static IReadOnlyList<SessionProfile> Detect(IEnumerable<string> candidateConfigDirs, Func<string, bool> directoryExists)
    {
        ...
        foreach (var configDir in candidateConfigDirs)
        {
            ...
            profiles.Add(new SessionProfile(LabelFromDirectoryName(configDir), ClaudePluginProfile.Create(configDir, executablePath: null)));
        }
