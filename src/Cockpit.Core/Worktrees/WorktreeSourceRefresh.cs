...
// operator about it. `Notice` is null when nothing happened worth mentioning — the branch was already
...
//
// `Outcome`: Which of the states the update attempt ended in.
// `BehindCount`: How many commits the source branch was behind its upstream, as last known; 0 when there was nothing to be behind of.
// `Upstream`: The upstream ref the source was measured against (e.g. "origin/main"), or null when the branch tracks nothing.
// `Notice`: The operator-facing sentence, or null when there is nothing worth saying.
// `ForkCommit`:
...
// (AC-376). `Outcome` says which of the two happened. Null when the fork base is simply that HEAD, and
