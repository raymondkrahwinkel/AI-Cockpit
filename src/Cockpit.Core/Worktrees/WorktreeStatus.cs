...
public sealed record WorktreeStatus(WorktreeRecord Record, bool Exists, bool HasUncommittedChanges, int StrandableCommits)
{
    ...
    // worktree that is not one. Nothing can be measured about it, so it is not `HasUncommittedChanges`
    ...
    // `IsClean`: "clean" is a measurement, and there is nothing here to measure.
