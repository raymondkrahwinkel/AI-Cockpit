...
// What a session does with one `ProjectResource` row (AC-483). Three roles, not one, because they name
// three different verbs a session performs on what is otherwise the same shape of data (a reference and a label):
// - <description>`Memory` is read *and* written back to — a session goes there to learn
// what the project already knows, and leaves what it just learned for the next one.</description>
// - <description>`Instructions` is read and obeyed — standing instructions a session follows, the
// way `Project.BehaviorPrompt` already is, except kept in a file rather than typed into the project
// itself.</description>
// - <description>`Reference` is looked up, never written to and never obeyed — material a session
// consults when it needs an answer, the way an `ProjectInfoField` row already is for the operator.</description>
