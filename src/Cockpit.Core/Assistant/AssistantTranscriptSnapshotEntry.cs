...
// `AssistantTranscriptEntry` (AC-544's read-gateway projection, unfit to reconstruct a `ToolUse` chip).
// Provider-neutral and decoupled from Cockpit.App's TranscriptEntryViewModel — Core cannot see that assembly.
// `Kind` is that enum's name as a string, so this type does not have to know it.
