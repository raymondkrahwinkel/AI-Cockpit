...
internal sealed class SecretProtectionService : ISecretProtectionService, ISingletonService
{
    ...
    public void Relock() =>
        ...
        // of never having unlocked at startup. Relock (not Lock): encryption stays on, so the config write seam must
        // refuse writes while locked rather than write the secrets in the clear the way it does when encryption is off.
        _keyHolder.Relock();
