...
/// into zero-or-more typed <see cref="SessionEvent"/>s.
/// </summary>
/// <remarks>
...
internal static class ClaudeStreamJsonParser
{
    ...
    /// </summary>
    public static IEnumerable<SessionEvent> ParseLine(string line)
    {
        ...
        IEnumerable<SessionEvent> events = type switch
        {
    ...
    public static SessionEvent? TryParseLine(string line) => ParseLine(line).FirstOrDefault();
    ...
    private static IEnumerable<SessionEvent> ParseSystem(JsonElement root, string? sessionId)
    {
    ...
    private static SessionEvent? ParseAssistantContentBlock(JsonElement block, string? sessionId)
    {
    ...
    public static IEnumerable<SessionEvent> ParseAssistantContentBlocks(JsonElement root, string? sessionId)
    {
    ...
    private static IEnumerable<SessionEvent> ParseUser(JsonElement root, string? sessionId)
    {
        if (!root.TryGetProperty("message", out var message) ||
            !message.TryGetProperty("content", out var content) ||
            content.ValueKind != JsonValueKind.Array)
    ...
    private static IEnumerable<SessionEvent> ParseStreamEvent(JsonElement root, string? sessionId)
    {
    ...
    private static IEnumerable<SessionEvent> ParseRateLimitEvent(JsonElement root, string? sessionId)
    {
        var uuid = root.TryGetProperty("uuid", out var uuidProp) ? uuidProp.GetString() : null;
    ...
    private static SessionEvent ParseResult(JsonElement root, string? sessionId)
    {
