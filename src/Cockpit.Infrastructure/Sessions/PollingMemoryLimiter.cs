...
// macOS `ISessionMemoryLimiter` (AC-661): no cgroups and no job objects, so the tree's RSS is polled. It used to
// kill the session outright once the tree passed its cap; AC-692 retired that — Cockpit no longer decides this for
// the operator, only reports it, and `CockpitViewModel`'s own resource poll is what actually tells them, with a
// toast that names the session and a kill button on it. `PollingMemoryLimiterTests` covers the logic against a
// fake process table; what no test here can cover is whether `ps` reports these figures on real macOS hardware,
// since there is no Mac to check on (the same blind spot as AC-57).
