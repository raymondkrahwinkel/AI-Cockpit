...
// The concrete roster behind `IWorkspaceAgentCoordinator` (AC-391): one flat set of enrolled pane
// ids, each carrying whether that pane has agreed to be woken (AC-395). A
// `ConcurrentDictionary{TKey,TValue}` rather than a locked `HashSet{T}`, since MCP
