...
// `COCKPIT_MCP_KEY` instead of the shared app key, so a request that reaches a cockpit endpoint can be
// attributed to the exact session that made it — the transport-verified identity the consent broker scopes remember
// decisions on, rather than the `session` value the agent declares (which it can forge to ride another pane's
// approvals).
//
...
// live sibling's token with it. `PluginSessionDriverAdapter.DisposeAsync` covers plugin-backed, embedded and
// delegated sessions; a TTY session's returned process revokes on its own `Dispose` (`TtyLauncher`
// mints, its `TtyProcessOwningSessionFiles` wrapper revokes); the local-model tool loop's session revokes on
// its own `DisposeAsync` (`McpToolProvider.ConnectAsync` mints, its `McpToolSession` revokes).
