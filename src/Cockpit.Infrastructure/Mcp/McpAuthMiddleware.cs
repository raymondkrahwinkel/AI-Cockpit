...
// per-session token) or a per-session token from the `SessionMcpKeyring` (AC-89). A per-session token
// additionally names the session: the middleware stamps that verified pane id onto `McpRequestContext`
