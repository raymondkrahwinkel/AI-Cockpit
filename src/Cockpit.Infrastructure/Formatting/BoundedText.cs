...
/// audit trails before they write a line (<see cref="Auditing.JsonlAuditLog{T}"/>), the agent roster before it repeats
/// one session's name and statusline into another's context (<c>AgentsMcpTools</c>) — and all of them want the same
/// answer to "what does the last character do at the boundary".
/// </summary>
internal static class BoundedText
{
    /// <summary>
    /// Trims <paramref name="text"/> to <paramref name="maxLength"/> characters plus an ellipsis, or returns it
