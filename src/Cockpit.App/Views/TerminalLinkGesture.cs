...
{
    // Ctrl held, left button, and the *first* press of the sequence. The click count is what AC-560 was
    // about: without it a Ctrl+double-click delivers two presses over the same link and opens the URL twice.
    // `TerminalControl.ActivateLink` gates its own path on `_clickCount == 1` for the same reason; this
