...
public sealed partial class CloneFromGitUrlDialogViewModel : ObservableObject
{
    ...
    // Raised when the dialog should close: the local clone path on success, or `null` on cancel.
    public event Action<string?>? CloseRequested;

    // The repository URL to clone — HTTPS, or an SSH remote the host's ssh config already handles.
    [ObservableProperty]
    private string _url = string.Empty;

    // Where the clone lands. Pre-filled from the URL with the managed `host/org/repo` default and kept in step
    ...
    // Clone is actionable when a URL is typed and none is already running.
