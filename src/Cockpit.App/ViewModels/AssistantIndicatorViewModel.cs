...
// A direct binding would have tied this file to one host and made the companion-window reuse a second, drifting copy
// instead of the same control (AC-543, AC-238).
public partial class AssistantIndicatorViewModel : ViewModelBase
{
    ...
    // Defaults to `true` so the component renders on its own — in a scene, a test, or AC-238's companion window —
    // without every consumer having to remember to switch it on (AC-542).
    [ObservableProperty]
    ...
    // Null when nothing is being prepared; the progress is null too whenever the step carries no total to measure
    // against, and then the chip shows the words alone rather than a bar parked at an invented position.
    [ObservableProperty]
    [NotifyPropertyChangedFor(nameof(Label))]
    [NotifyPropertyChangedFor(nameof(Detail))]
    ...
    // Fed in by the host — which derives it from `VoiceSettings.OpenMicEnabled` rather than storing a second flag with
    // the same meaning — so the picker below always shows the mode actually in effect, never a locally-guessed one.
    [ObservableProperty]
    ...
    // Fed in rather than owned here: the setting is persisted by the host, and this view model only needs to know it to
    // decide whether picking AlwaysOn should ask first (criterion 18).
    [ObservableProperty]
    private bool _alwaysOnCostAcknowledged;

    // Local UI state rather than something the host tracks: it exists only for the moment between the pick and the
    // operator's answer, and closes itself (`CancelAlwaysOnConfirmation`) without needing a round trip through
    // settings.
    [ObservableProperty]
    ...
    // confirmation the operator would otherwise get, and criterion 5 asks that it be visible without opening Options —
    // the chip is the only surface that is always on screen while the assistant is on (AC-575).
    [ObservableProperty]
    private bool _isConsentBypassActive;

    // The host decides what that means; this view model only reports the click.
    public event EventHandler? Clicked;

    // Where that lands is the host's decision, not this view model's — today that means flipping
    // `VoiceSettings.OpenMicEnabled`, since the mode itself is not stored anywhere; this view model only lives on the
    // surface the operator picks from.
    public event EventHandler<AssistantListeningMode>? ListeningModeSelected;

    // The primary line — what to call this state out loud.
    ...
    // Every other assistant-side state names the assistant, so the chip still reads "the assistant" rather than only a
    // colour.
    public string? Detail => Activity switch
    {
        ...
        // The step's own words are the headline above; this line is the number, and a step without a total shows
        // nothing here rather than a made-up one.
    ...
    // The key hint shown at the right of the chip (mockup's `.key` badge) — only for the states that actually have a
    // key bound to them right now; `null` hides it rather than showing an empty badge.
    ...
    // Whether the mic glyph is the badge's icon — every state except Thinking, Speaking and Unavailable, which draw
    // their own (a thought bubble, a speaker, an alert triangle).
    ...
    // listening switch in the two resting stands, the key badge in the states that are mid-something.
    ...
    // One number rather than the pill's scrolling history, because the badge is a circle and because this is the one
    // form that survives into the collapsed rail — where the badge is all there is left of the chip.
    [ObservableProperty]
    ...
    public void PushLevel(double level)
    {
        ...
        // Peak with decay rather than the raw frame.
    ...
    // The first time, this only opens the inline confirmation (criterion 18) instead of committing — `ConfirmAlwaysOn`
    // is what actually raises `ListeningModeSelected` then.
    [RelayCommand]
    ...
    // Answers the inline confirmation: acknowledges the cost and commits the pick, in that order — so a host reading
    // the event already sees `AlwaysOnCostAcknowledged` as true and never re-asks.
    [RelayCommand]
    ...
    // It delegates to the two picks rather than raising `ListeningModeSelected` itself, so AlwaysOn's one-time
    // confirmation (criterion 18) still stands in front of it — a switch that skipped the explanation would be a
    // quieter control that gives away more than the two buttons did.
    [RelayCommand]
    ...
    // The switch flips itself the moment it is clicked, but the pick only lands once the host has applied it — and it
    // may not land at all (the AlwaysOn confirmation is waiting, or the operator cancels it).
