...
/// <c>Dispatcher.UIThread.Post</c> each. Here the producer only enqueues, and asks for a drain when no drain is
...
/// original stack, that a per-event post used to reach. What "itself" covers is one <em>applied</em> event, which
...
internal sealed class SessionEventQueue
{
    ...
    /// <param name="apply">Applies one event to the view model; runs on whatever thread <paramref name="post"/> lands on.</param>
    /// <param name="post">
    /// Test seam, the same shape as <c>ToastHostViewModel</c>'s scheduler and <c>DevPluginReloadWatcher</c>'s
    /// debounce: null takes the real <see cref="Dispatcher.UIThread"/>, a test hands in a manual pump so the drain
    ...
    /// <summary>Takes one event from the runtime's pump thread and makes sure a drain is coming.</summary>
    ...
    /// as the action <see cref="Enqueue"/> posted; a test's manual pump runs the same method.
    /// </summary>
    internal void Drain()
    {
