...
// start a session (#31/#32). `Kind` is picked inside the dialog itself and tells the
// cockpit which session type to create. Both kinds carry all four remaining fields: for TTY these are
// launch-only start defaults passed as CLI flags (`--permission-mode`/
// `--dangerously-skip-permissions`, `--model`, `--effort`) — once running, the real TUI
// owns any live switching itself (`/model`, `/effort`, Shift+Tab), since TTY mode has no
// control channel.
//
// `EnabledMcpServerNames`:
// The per-session MCP-server selection (#44) picked in the dialog's checklist of the shared registry's
// enabled servers — `null` when the dialog found no registry servers to offer, meaning
// no session-level restriction applies on top of the registry's own enabled/scope filtering. Consumed by
// the Claude SDK/local-model tool-loop (`McpToolProvider`) and the Claude-CLI `--mcp-config`
// fan-out (`ClaudeCliProcess`); the TTY driver does not fan the registry out at all today, so this
// has no effect there.
// `WorkingDirectory`:
// An optional per-session working directory chosen in the dialog (e.g. a project folder), overriding the
// global `Claude:WorkingDirectory` option for this one session — the directory `claude` is
// launched in, for both the SDK process and the TTY pty. `null`/blank keeps the global
// default (the configured option, else the app's current directory).
// `PluginTtyOptions`:
// The start defaults chosen for a *plugin* TTY provider's own declared options (Codex's sandbox
// policy, say) — keyed exactly as that provider's `TtyProviderRegistration.Options` declared them.
// `null` for a Claude session (which carries its start defaults through
// `Mode`/`Model`/`Effort` instead) or a profile with no declared
// options. The two never both apply to the same launch: `Mode`/`Model`/`Effort`
...
// `WorkingDirectory` is a git repository — a per-session choice made in the dialog next to the
...
// session — chosen in the dialog and shown only for an SDK session. `null` keeps the profile
// default (the New-session dialog seeds it from there). Ignored for a TTY session, which has no reading level.
// `ProjectId`:
// The project this session works on (AC-163), or `null` for one belonging to none. Carried so the
...
// the project's behaviour under it, already resolved by `SessionStartDefaults`. `null` (or
// blank) does not mean "appends nothing" any more (AC-544): `SdkLaunchOptionsWithInstructions` and
// `TtyLaunchOptionsWithInstructions` fall back to `Cockpit.Core.Sessions.AgentStatusSystemPrompt.Default`
...
{
    // Whether `SessionName` was put together by the cockpit rather than chosen by anybody — "Cockpit 2",
    ...
    //
    // It rides along here so the one place that decides — `AddSession` — can be told, instead of every route
    ...
    // `options` carrying the resolved instructions under the well-known append-system-prompt key,
    ...
    // its own.
    //
    // AC-544 criterion 5: `AgentStatusSystemPrompt.Default` rides *alongside* a profile's own
    ...
    // `ClaudeTtyProvider._AppendedInstructions` joins it to whatever the profile resolved — a profile with an
    ...
    //
    // *Still replaceable per profile*, in the way a system prompt is replaceable at all: the profile's own
