...
// all": a null `CategoryName` is the single group an install where nobody uses categories gets, and it
// draws with no heading whatsoever — the list looks exactly as it did before this existed. Once at least one
// project carries a category, every real category becomes its own group in `ProjectSettings.CategoryOrder`'s
