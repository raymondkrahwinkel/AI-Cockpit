...
/// active plugin provides for itself (<see cref="IPluginMcpProvider"/>). This is what the fan-out and the
/// New-session checklist read, so plugin-owned servers are offered and per-session uncheckable alongside registry
/// ones — while the MCP-servers manager keeps reading <see cref="IMcpServerStore"/> directly and so never lists
/// them.
/// </summary>
internal sealed class McpServerCatalog(IMcpServerStore store, PluginManager pluginManager, ILogger<McpServerCatalog> logger)
    : IMcpServerCatalog, ISingletonService
{
    public async Task<IReadOnlyList<McpServerConfig>> GetServersAsync(CancellationToken cancellationToken = default)
    {
        ...
        var pluginServers = pluginManager.Plugins
            .OfType<IPluginMcpProvider>()
            .SelectMany(_ServersOf)
            .Select(PluginMcpMapping.ToServerConfig)
            .ToList();

        // A plugin owns its own server names, so its live answer wins over any registry entry of the same name —
        // the case that arises for one start after upgrade, before the pre-AC-11 push entries are reconciled away.
