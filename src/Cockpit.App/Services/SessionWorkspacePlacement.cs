...
// *Why this exists.* The same rule was written out three times, independently:
// `WorkspaceAgentGateway`'s `_ResolveWorkspaceId`, `PaneWorkspaceDirectory`'s own
// inline copy, and `CockpitViewModel.BelongsToActiveWorkspace`. Each said the same thing — an unassigned
...
//
// *The third session kind.* The cockpit had two: a pane in a workspace, and a headless delegated task with
...
// session it cannot account for. So the assistant resolves to `null` — no workspace at all —
// and every caller gets that answer explicitly.
//
// `null` is not a new case for the callers: it is the answer they already got when a session had
