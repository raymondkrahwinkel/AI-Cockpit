...
/// <param name="Detail">The tool's raw text output, present only when <paramref name="Outcome"/> is <see cref="McpProbeOutcome.Success"/>.</param>
public sealed record McpProbeResult(McpProbeOutcome Outcome, string? Detail = null)
{
    /// <summary>The call could not be completed — see <see cref="McpProbeOutcome.Failed"/>.</summary>
    public static McpProbeResult Failed { get; } = new(McpProbeOutcome.Failed);

    /// <summary>The server needs a sign-in — see <see cref="McpProbeOutcome.NotSignedIn"/>.</summary>
    public static McpProbeResult NotSignedIn { get; } = new(McpProbeOutcome.NotSignedIn);

    /// <summary>The tool reported the value does not resolve — see <see cref="McpProbeOutcome.NotFound"/>.</summary>
