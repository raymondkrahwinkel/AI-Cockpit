...
/// mirror of <c>Cockpit.Core.Sessions.SessionEvent</c>, trimmed to the subset a third-party HTTP provider
/// can actually produce (no Claude-CLI-only thinking/status/rate-limit events). The host's driver adapter
/// maps each subtype to its <c>SessionEvent</c> counterpart so the rest of the app sees one event
