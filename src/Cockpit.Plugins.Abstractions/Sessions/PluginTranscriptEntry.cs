...
{
    /// <summary>What the operator (or a sent prompt) said.</summary>
    UserText,

    /// <summary>The model's prose.</summary>
    AssistantText,

    /// <summary>The model calling a tool — the name, and whatever the provider can say about the arguments.</summary>
    ToolUse,

    /// <summary>What a tool call returned, carried on the call it belongs to where the provider can pair them.</summary>
    ToolResult,

    /// <summary>A reasoning/extended-thinking block.</summary>
    Thinking,

    /// <summary>An error the transcript recorded.</summary>
    Error,
}
...
/// </summary>
/// <param name="Kind">What this row is.</param>
/// <param name="Text">The row's text: the message, the thinking, or a tool call's name and arguments.</param>
/// <param name="ToolResult">On a <see cref="PluginTranscriptEntryKind.ToolUse"/> row, what that call returned, when the provider can pair the two. Null everywhere else.</param>
...
/// </summary>
/// <param name="Entries">The last rows asked for, oldest first.</param>
/// <param name="TotalEntries">How many rows the transcript holds in all — never less than <see cref="Entries"/>'s count.</param>
public sealed record PluginTranscriptSlice(IReadOnlyList<PluginTranscriptEntry> Entries, int TotalEntries)
{
    /// <summary>Nothing read: what a provider reports for a session with no transcript it can name, and what the interface default answers.</summary>
