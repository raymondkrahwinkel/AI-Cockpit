...
/// <summary>What came of <see cref="ISharedProjectSource.WriteBackAsync"/> (AC-247) — the write-side mirror of <see cref="SharedProjectListResult"/>/<see cref="SharedProjectBindingResult"/>'s own outcome idiom.</summary>
public enum SharedProjectWriteBackOutcome
{
    /// <summary>The edit landed. <see cref="SharedProjectWriteBackResult.Checksum"/> is the new token for a follow-up write.</summary>
    Success,
    ...
    /// <summary>The operator's role does not allow writing here. Never retry with the same edit; show <see cref="SharedProjectWriteBackResult.Error"/> as the reason.</summary>
    PermissionDenied,

    /// <summary>Anything else — unreachable, malformed response, an error the source could not further classify.</summary>
    Failed,
}

/// <summary>See <see cref="SharedProjectWriteBackOutcome"/>.</summary>
    ...
    SharedProjectWriteBackOutcome Outcome, string? Checksum = null, string? Error = null, SharedProjectBinding? LatestSnapshot = null)
{
    ...
    /// <param name="latest">A fresh read of the source's current state, taken while answering this call — never the caller's own stale edit.</param>
