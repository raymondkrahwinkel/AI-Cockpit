...
- fixed: an Autopilot epic run's next Ready sub now carries its own description and URL — previously only its
  title and stage came along, so a template placeholder like `{{issue.url}}` or `{{issue.description}}` resolved
  blank for an epic-picked sub even though the same placeholders fill in fine for a directly clicked issue.
