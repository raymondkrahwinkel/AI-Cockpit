...
- added: for plugin authors, the plugin SDK gains its window half next to the existing contract: a plugin can
...
- added: each cluster in the Kubernetes plugin's settings now has a consent mode — Always ask (unchanged),
  ...
  them a change asks as before; an Argo CD sync, a Helm upgrade or rollback that touches cluster-scoped
  resources, and exec or port-forward — a shell or tunnel reaches other namespaces — always ask). It is tied to the kubeconfig and context the cluster points at, so re-pointing
...
- added: a layout that will not settle is now stopped instead of taking the whole window down with it. When the
  ...
  deliberately far above anything real work needs: the heaviest healthy layout measured stands still for a little
  over a tenth of a second, against the twenty seconds it takes to trip this. Set `COCKPIT_LAYOUT_CUTOFF_SAMPLES` to change how
...
- changed: for plugin authors, GitHub Issues and GitHub Pull Requests now follow Git status into the two-part
  ...
  before.
- changed: for plugin authors, a plugin's backend part can now start the sessions its workspace shows and hold
  them by pane id, while the workspace places their live views. Autopilot's runs and its planning session now
  start this way; the Autopilot workspace looks and works as before.
...
- fixed: a terminal session that sits quiet for two minutes while a tool or a slow MCP server answers no longer
  ...
  has always said. A session that is genuinely finished still turns Done — its own terminal does not count as
  work — and one running something that never ends, a dev server or a `tail -f`, turns Done after ten minutes of
  silence rather than sitting on "working" for good and asking every time you try to close it. A short command
  flickering past cannot flip the sidebar back and forth either: a session is called quiet only once two
  consecutive readings say so.
...
- fixed: the view no longer jumps up and back down while a reply is arriving. A bullet list, a numbered list, a
  ...
  grew, the panel kept re-guessing how tall the whole conversation was and repeatedly drew a completely
  different part of it: on one frame in eight the end of the answer was nowhere on screen, and the next frame
  put it back. Long blocks like these now arrive in parts, so the row that is still growing stays small. A
  numbered list that arrives in parts keeps counting where the part before it stopped instead of starting again
  at 1, and a table that arrives in parts keeps every part's columns lined up with the rest.
...
- fixed: a long conversation no longer grows the cockpit's memory the way it used to. Every transcript row the
  assistant's chat or a session pane scrolled into view left a failed lookup behind — in a twelve-hour run that was
  eleven and a half thousand of them, 99% of every warning the app logged, and it tracked the heap climbing past
  6 GB. Rows now carry the session they belong to instead of searching for it while they are being built.
